...
def text_complete_last_message(history, stop_words_ids):
    ...
    output = model.generate(
        input_ids, stop_words_ids=stop_words_ids).tolist()[0]
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if '上传了' in query:
        ...
        end_index = query.rfind('}]') + 2
        query = query[start_index:end_index].strip()
        query = query.replace("'", '"')
        query = tokenizer.from_list_format(json.loads(query))
    print(query)
    print("=========================")
    if request.stream:
        if request.functions:
    ...
    if query is _TEXT_COMPLETION_CMD:
        response = text_complete_last_message(
            history, stop_words_ids=stop_words_ids)
    else:
